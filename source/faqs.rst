...
Open OnDemand allows system administrators to quickly and easily connect many 
...
system to manage user groups and levels of access. Open OnDemand is trusted and used 
...
developed for their systems. Contained in the official Open OnDemand distribution 
...
a monthly virtual meeting with the Open OnDemand development team to help diagnose and solve persistent issues.

How many people are typically required to maintain an institutional instance? How does this scale?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Institutions typically find that Open OnDemand configuration and maintenance is easily handled by a 
...
light work. Scaling issues could arise if the login node hosting your Open OnDemand instance is overloaded, and may 
