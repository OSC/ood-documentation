...
and administrators can now edit the ``ood_portal.yml`` configuration and then restart httpd
with systemctl and apache will read the new configuration, because a new ood-portal.conf
will have been written just before restarting.
...
clean a specific users' PUN. For example ``nginx_stage nginx_clean -u johndoe`` would only
clean johndoe's PUN and disregard all the others.
