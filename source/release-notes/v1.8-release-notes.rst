...
- `Shell hosts are now protected behind allow-lists`_
...
If your site also uses `XDMoD`_ with the Job Performance module (formerly called SUPReMM) for job performance analysis,
...
drop-down select menu item at the top of the form where users can choose between
the two options.
...
values the user set are stored in a cache file that is used to prepopulate the web
...
Shell hosts are now protected behind allow-lists
................................................

Shell hosts are now protected behind allow-lists. This means administrators now need
to explicitly add hosts to an allow-list for users to be able to access them through
the shell app.

Any login host of a cluster configuration will be allowed, assuming that cluster is not
configured as "hidden" in the metadata section of the cluster configuration.
...
Related Ruby gems for the micro-web-framework "Sinatra" were added to the Dashboard Gemfile to ensure they are included in the ``ondemand-gems`` RPM.
