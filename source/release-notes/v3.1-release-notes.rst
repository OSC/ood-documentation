...
to work correctly with OnDemand 3.1.9 or higher. As of February 2024,
...
We implemented changes the location of the :ref:`user settings file <user_settings_file>`
...
to accommodate sites that have separate HOME directories for each cluster.
...
Open OnDemand version 3.0 was the last release to support RedHat/CentOS 7.

Open OnDemand versions 3.1 and above do not support RedHat/CentOS 7.
...
Open OnDemand now ships it's own ``path_selector`` form widget.
...
supplying a global log configuration in a separate file.

The second is to allow for custom directives.  You can now supply
